...
export default class extends Controller {
  ...
  updateFromWelding(event) {
    ...
    fetch(`/weldings/${weldingId}`, {
      ...
      .then((welding) => {
        ...
        const batchNumberField = row.querySelector(
          '[data-welding-certificate-search-target="batchNumber"][data-field="batch_number"]'
        );
        const batchNumber1Field = row.querySelector(
          '[data-welding-certificate-search-target="batchNumber"][data-field="batch_number1"]'
        );
        const certificateIdField = row.querySelector(
          '[data-welding-certificate-search-target="certificateId"][data-field="batch_number"]'
        );
        const certificateId1Field = row.querySelector(
          '[data-welding-certificate-search-target="certificateId"][data-field="batch_number1"]'
        );
  ...
  search() {
    ...
    if (!query) {
      ...
      // Clear the certificate ID when batch number is empty
  ...
  clearFields() {
    ...
    row
      .querySelectorAll(
        '[data-welding-certificate-search-target="batchNumber"]'
      )
      .forEach((field) => {
        field.value = "";
      });
    row
      .querySelectorAll(
        '[data-welding-certificate-search-target="certificateId"]'
      )
      .forEach((field) => {
        field.value = "";
      });
  ...
  showResults(certificates) {
    if (!certificates.length) {
      this.searchResultsTarget.innerHTML =
        '<div class="list-group"><div class="list-group-item">No results found</div></div>';
      return;
    }
  ...
  selectFromList(event) {
    ...
    const field = event.target
      .closest('[data-controller="welding-certificate-search"]')
      .querySelector('[data-welding-certificate-search-target="batchNumber"]')
      .dataset.field;
